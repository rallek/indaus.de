 ...
 */
function rKReferenceInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('rkreferencemodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
 ...
 */
function rKReferenceResetUploadField(fieldName)
{
 ...
 */
function rKReferenceInitUploadField(fieldName)
{
    jQuery('#' + fieldName + 'ResetVal').click( function (event) {
        event.stopPropagation();
        rKReferenceResetUploadField(fieldName);
    }).removeClass('hidden');
}
...
function rKReferenceTriggerFormValidation()
{
    rKReferenceExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function rKReferenceHandleFormSubmit (event) {
    if (triggerValidation) {
        rKReferenceTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function rKReferenceInitEditForm(mode, entityId)
{
    if (jQuery('.rkreference-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.rkreference-edit-form').first();
    ...
    editForm.submit(rKReferenceHandleFormSubmit);
