...
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Attached image'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='attachedImage' from=$items}
            <option value="{$attachedImage.id}"{if $selectedId eq $attachedImage.id} selected="selected"{/if}>{$attachedImage->getTitleFromDisplayPattern()}</option>
        {foreachelse}
...
<p>
    ...
    <select id="{$baseID}Sort" name="sort"{$rightSide}>
        ...
        <option value="addedImage"{if $sort eq 'addedImage'} selected="selected"{/if}>{gt text='Added image'}</option>
        <option value="shortDescription"{if $sort eq 'shortDescription'} selected="selected"{/if}>{gt text='Short description'}</option>
