...
abstract class AbstractInformationRepository extends TranslationRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\InfoModule\Entity\InformationEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKInfoModule', 'entity' => 'information'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\InfoModule\Entity\InformationEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKInfoModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'informations', 'userid' => $userId];
     ...
     *
     * @return array|informationEntity retrieved data array or informationEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved informationEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.theDescription LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numInformations';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\InfoModule\Entity\InformationEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of informations to exclude (optional)
     *
     * @return boolean result of this check, true if the given information does not already exist
     */
    ...
    public function getQueryFromBuilder(QueryBuilder $qb)
    {
        ...
        $featureActivationHelper = ServiceUtil::get('rk_info_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::TRANSLATIONS, 'information')) {
