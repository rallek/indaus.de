...
use RK\InfoModule\InfoEvents;
use RK\InfoModule\Event\FilterInformationEvent;
...
abstract class AbstractEntityLifecycleListener implements EventSubscriber, ContainerAwareInterface
{
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\InfoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\InfoModule\\InfoEvents::' . strtoupper($entity->get_objectType()) . '_PRE_REMOVE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
        
        // delete workflow for this entity
        $workflowHelper = $this->container->get('rk_info_module.workflow_helper');
    ...
    public function postRemove(LifecycleEventArgs $args)
    {
        ...
        $logArgs = ['app' => 'RKInfoModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $objectType, 'id' => $objectId];
        $logger->debug('{app}: User {user} removed the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\RK\\InfoModule\\Event\\Filter' . ucfirst($objectType) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\InfoModule\\InfoEvents::' . strtoupper($objectType) . '_POST_REMOVE'), $event);
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\InfoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\InfoModule\\InfoEvents::' . strtoupper($entity->get_objectType()) . '_PRE_PERSIST'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        ...
        $objectId = $entity->createCompositeIdentifier();
        $logger = $this->container->get('logger');
        $logArgs = ['app' => 'RKInfoModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $entity->get_objectType(), 'id' => $objectId];
        $logger->debug('{app}: User {user} created the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\RK\\InfoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\InfoModule\\InfoEvents::' . strtoupper($entity->get_objectType()) . '_POST_PERSIST'), $event);
    }
    ...
    public function preUpdate(PreUpdateEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\InfoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\InfoModule\\InfoEvents::' . strtoupper($entity->get_objectType()) . '_PRE_UPDATE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\InfoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\InfoModule\\InfoEvents::' . strtoupper($entity->get_objectType()) . '_POST_UPDATE'), $event);
    }
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\InfoModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\InfoModule\\InfoEvents::' . strtoupper($entity->get_objectType()) . '_POST_LOAD'), $event);
    }
    ...
    protected function isEntityManagedByThisBundle($entity)
    {
        ...
        return ($entityClassParts[0] == 'RK' && $entityClassParts[1] == 'InfoModule');
    }
}
