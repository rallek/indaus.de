...
namespace RK\ReferenceModule\Menu\Base;
...
class AbstractItemActionsMenu implements ContainerAwareInterface
{
    ...
    public function menu(FactoryInterface $factory, array $options)
    {
        ...
        if ($entity instanceof AttachedImageEntity) {
            $component = 'RKReferenceModule:AttachedImage:';
            $instance = $entity['id'] . '::';
            $routePrefix = 'rkreferencemodule_attachedimage_';
            ...
            if ($permissionApi->hasPermission($component, $instance, ACCESS_EDIT)) {
                ...
                $menu[$this->__('Edit')]->setLinkAttribute('title', $this->__('Edit this attached image'));
                $menu->addChild($this->__('Reuse'), [
                    'route' => $routePrefix . $routeArea . 'edit',
                    'routeParameters' => ['astemplate' => $entity['id']]
                ])->setAttribute('icon', 'fa fa-files-o');
                $menu[$this->__('Reuse')]->setLinkAttribute('title', $this->__('Reuse for new attached image'));
            }
            if ($permissionApi->hasPermission($component, $instance, ACCESS_DELETE)) {
                ...
                $menu[$this->__('Delete')]->setLinkAttribute('title', $this->__('Delete this attached image'));
            }
            if ($context == 'display') {
            ...
            $relatedComponent = 'RKReferenceModule:Activity:';
            $relatedInstance = $entity['id'] . '::';
            if ($isOwner || $permissionApi->hasPermission($relatedComponent, $relatedInstance, ACCESS_EDIT)) {
                $title = $this->__('Create activity');
                $menu->addChild($title, [
                    'route' => 'rkreferencemodule_activity_' . $routeArea . 'edit',
                    'routeParameters' => ['activity' => $entity['id']]
                ])->setAttribute('icon', 'fa fa-plus');
                $menu[$title]->setLinkAttribute('title', $title);
            }
        }
        if ($entity instanceof ActivityEntity) {
            $component = 'RKReferenceModule:Activity:';
            $instance = $entity['id'] . '::';
            $routePrefix = 'rkreferencemodule_activity_';
            $isOwner = $currentUserId > 0 && null !== $entity->getCreatedBy() && $currentUserId == $entity->getCreatedBy()->getUid();
            ...
            if ($permissionApi->hasPermission($component, $instance, ACCESS_EDIT)) {
                ...
                $menu[$this->__('Edit')]->setLinkAttribute('title', $this->__('Edit this activity'));
                $menu->addChild($this->__('Reuse'), [
                    'route' => $routePrefix . $routeArea . 'edit',
                    'routeParameters' => ['astemplate' => $entity['id']]
                ])->setAttribute('icon', 'fa fa-files-o');
                $menu[$this->__('Reuse')]->setLinkAttribute('title', $this->__('Reuse for new activity'));
            }
            if ($permissionApi->hasPermission($component, $instance, ACCESS_DELETE)) {
                $menu->addChild($this->__('Delete'), [
                    'route' => $routePrefix . $routeArea . 'delete',
                ...
                $menu[$this->__('Delete')]->setLinkAttribute('title', $this->__('Delete this activity'));
            }
            if ($context == 'display') {
                $title = $this->__('Back to overview');
                $menu->addChild($title, [
