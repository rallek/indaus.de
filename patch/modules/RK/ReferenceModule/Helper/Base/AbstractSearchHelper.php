...
abstract class AbstractSearchHelper implements SearchableInterface
{
     ...
     * @param ReferenceFactory $entityFactory EntityFactory service instance
    ...
    {
        if (!$this->permissionApi->hasPermission('RKReferenceModule::', '::', ACCESS_READ)) {
            return '';
        }
    
        $templateParameters = [];
    
        $searchTypes = ['attachedImage', 'activity'];
        foreach ($searchTypes as $searchType) {
            $templateParameters['active_' . $searchType] = !isset($args['rKReferenceModuleSearchTypes']) || in_array($searchType, $args['rKReferenceModuleSearchTypes']);
        }
    
        return $this->templateEngine->renderResponse('@RKReferenceModule/Search/options.html.twig', $templateParameters)->getContent();
    }
    ...
    public function getResults(array $words, $searchType = 'AND', $modVars = null)
    {
        ...
        if (!is_array($searchTypes) || !count($searchTypes)) {
            if ($this->request->isMethod('GET')) {
                $searchTypes = $this->request->query->get('rKReferenceModuleSearchTypes', []);
            } elseif ($this->request->isMethod('POST')) {
                $searchTypes = $this->request->request->get('rKReferenceModuleSearchTypes', []);
            }
        }
        ...
        foreach ($searchTypes as $objectType) {
            ...
            switch ($objectType) {
                case 'attachedImage':
                    ...
                    $whereArray[] = 'tbl.displayLanguage';
                    $whereArray[] = 'tbl.title';
                    $whereArray[] = 'tbl.addedImage';
                ...
                case 'activity':
                    ...
                    $whereArray[] = 'tbl.referenceImage';
                    $whereArray[] = 'tbl.copyrightReferenceImage';
                    $whereArray[] = 'tbl.teaserDescription';
                    $whereArray[] = 'tbl.activityDescription';
                    $whereArray[] = 'tbl.infoField1';
                    $whereArray[] = 'tbl.infoField2';
                    $whereArray[] = 'tbl.infoField3';
                    $whereArray[] = 'tbl.infoField4';
                    $whereArray[] = 'tbl.infoField5';
                    break;
            }
            ...
            $entitiesWithDisplayAction = ['attachedImage', 'activity'];
            ...
            foreach ($entities as $entity) {
                ...
                if (!$this->permissionApi->hasPermission('RKReferenceModule:' . ucfirst($objectType) . ':', $instanceId . '::', ACCESS_OVERVIEW)) {
                    continue;
                }
                ...
                $displayUrl = $hasDisplayAction ? new RouteUrl('rkreferencemodule_' . $objectType . '_display', $urlArgs) : '';
    
                $result = new SearchResultEntity();
                $result->setTitle($entity->getTitleFromDisplayPattern())
                    ->setText($description)
                    ->setModule('RKReferenceModule')
                    ->setCreated($created)
