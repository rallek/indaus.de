 ...
 *
 * This is the base repository class for attached image entities.
 */
abstract class AbstractAttachedImageRepository extends EntityRepository
{
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            $thumbRuntimeOptions = [];
            $thumbRuntimeOptions[$objectType . 'AddedImage'] = $imageHelper->getRuntimeOptions($objectType, 'addedImage', $context, $args);
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\ReferenceModule\Entity\AttachedImageEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKReferenceModule', 'entity' => 'attached image'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\ReferenceModule\Entity\AttachedImageEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKReferenceModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'attached images', 'userid' => $userId];
     ...
     *
     * @return array|attachedImageEntity retrieved data array or attachedImageEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved attachedImageEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.addedImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.shortDescription LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.displayLanguage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numAttachedImages';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\ReferenceModule\Entity\AttachedImageEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of attached images to exclude (optional)
     *
     * @return boolean result of this check, true if the given attached image does not already exist
     */
    ...
    {
        $qb->leftJoin('tbl.attachedImages', 'tblAttachedImages');
