...
function rKReferenceToday(format)
{
...
function rKReferenceReadDate(val, includeTime)
{
...
function rKReferenceValidateNoSpace(val)
{
    var valStr;
...
function rKReferenceValidateUploadExtension(val, elem)
{
 ...
 */
function rKReferenceExecuteCustomValidationConstraints(objectType, currentEntityId)
{
    jQuery('.validate-nospace').each( function() {
        if (!rKReferenceValidateNoSpace(jQuery(this).val())) {
    ...
    jQuery('.validate-upload').each( function() {
        if (!rKReferenceValidateUploadExtension(jQuery(this).val(), jQuery(this))) {
