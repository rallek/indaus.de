...
namespace RK\InfoModule\Form\Handler\Information\Base;

use RK\InfoModule\Form\Handler\Common\EditHandler;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'information';
        $this->objectTypeCapital = 'Information';
        $this->objectTypeLower = 'information';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the information yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'RKInfoModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of information area
        $codes[] = 'userIndex';
        // admin index page of information area
        $codes[] = 'adminIndex';
        // user list of informations
        $codes[] = 'userView';
        // admin list of informations
        $codes[] = 'adminView';
        // user list of own informations
        $codes[] = 'userOwnView';
        // admin list of own informations
        $codes[] = 'adminOwnView';
        // user detail page of treated information
        $codes[] = 'userDisplay';
        // admin detail page of treated information
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        $routePrefix = 'rkinfomodule_' . $this->objectTypeLower . '_' . $routeArea;
    
        // redirect to the list of informations
        $viewArgs = [];
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Information created.');
                } else {
                    $message = $this->__('Done! Information updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Information deleted.');
                break;
            default:
                $message = $this->__('Done! Information updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKInfoModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'information', 'id' => $entity->createCompositeIdentifier(), 'errorMessage' => $e->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        if ($this->request->getSession()->has('rkinfomoduleReferer')) {
            $this->request->getSession()->del('rkinfomoduleReferer');
        }
