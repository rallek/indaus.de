...
namespace RK\ReferenceModule\Entity\Repository\Base;
...
use RK\ReferenceModule\Helper\ImageHelper;

/**
 * Repository class used to implement own convenience methods for performing certain DQL queries.
 *
 * This is the base repository class for activity entities.
 */
abstract class AbstractActivityRepository extends EntityRepository
{
    
    /**
     * @var string The default sorting field/expression
     */
    protected $defaultSortingField = 'displayLanguage';
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            $thumbRuntimeOptions[$objectType . 'ReferenceImage'] = $imageHelper->getRuntimeOptions($objectType, 'referenceImage', $context, $args);
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbRuntimeOptions'] = $imageHelper->getCustomRuntimeOptions('', '', 'RKReferenceModule_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters = [];
        $parameters['activity'] = $this->getRequest()->query->get('activity', 0);
        $parameters['workflowState'] = $this->getRequest()->query->get('workflowState', '');
        $parameters['displayLanguage'] = $this->getRequest()->query->get('displayLanguage', '');
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\ReferenceModule\Entity\ActivityEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKReferenceModule', 'entity' => 'activity'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\ReferenceModule\Entity\ActivityEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKReferenceModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'activities', 'userid' => $userId];
     ...
     *
     * @return array|activityEntity retrieved data array or activityEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved activityEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= 'tbl.displayLanguage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.titleImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.copyrightTitleImage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.referenceImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.copyrightReferenceImage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.teaserDescription LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.activityDescription LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.infoField1 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.infoField2 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.infoField3 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.infoField4 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.infoField5 LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.displayLanguage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numActivities';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\ReferenceModule\Entity\ActivityEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of activities to exclude (optional)
     *
     * @return boolean result of this check, true if the given activity does not already exist
     */
    ...
    {
        $qb->leftJoin('tbl.activity', 'tblActivity');
