...
abstract class AbstractUserListener implements EventSubscriberInterface
{
     ...
     * @return void
     */
    public function __construct(ZkTranslatorInterface $translator, ReferenceFactory $entityFactory, CurrentUserApi $currentUserApi, LoggerInterface $logger)
    {
    ...
    public function delete(GenericEvent $event)
    {
        ...
        $repo = $this->entityFactory->getRepository('attachedImage');
        // set creator to admin (2) for all attached images created by this user
        $repo->updateCreator($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        // set last editor to admin (2) for all attached images updated by this user
        $repo->updateLastEditor($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        $logArgs = ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'attached images'];
        $this->logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
        
        $repo = $this->entityFactory->getRepository('activity');
        // set creator to admin (2) for all activities created by this user
        $repo->updateCreator($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        // set last editor to admin (2) for all activities updated by this user
        $repo->updateLastEditor($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        $logArgs = ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'activities'];
        $this->logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
    }
}
