...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction($objectType, $id, $source, $displayMode)
    {
        ...
        $component = 'RKReferenceModule:' . ucfirst($objectType) . ':';
        if (!$this->hasPermission($component, $id . '::', ACCESS_READ)) {
            return '';
        }
        
        $repository = $this->get('rk_reference_module.entity_factory')->getRepository($objectType);
        ...
        $additionalParameters = $repository->getAdditionalTemplateParameters($this->get('rk_reference_module.image_helper'), 'controllerAction', $contextArgs);
        $templateParameters = array_merge($templateParameters, $additionalParameters);
        
        return $this->render('@RKReferenceModule/External/' . ucfirst($objectType) . '/display.html.twig', $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@RKReferenceModule:css/style.css'));
        ...
        if (!$this->hasPermission('RKReferenceModule:' . ucfirst($objectType) . ':', '::', ACCESS_COMMENT)) {
        ...
        $form = $this->createForm('RK\ReferenceModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        $imageHelper = $this->get('rk_reference_module.image_helper');
        ...
        $output = $this->renderView('@RKReferenceModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
