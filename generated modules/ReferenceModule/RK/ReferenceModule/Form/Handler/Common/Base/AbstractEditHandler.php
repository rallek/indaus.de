...
namespace RK\ReferenceModule\Form\Handler\Common\Base;
...
use RK\ReferenceModule\Helper\ControllerHelper;
use RK\ReferenceModule\Helper\HookHelper;
use RK\ReferenceModule\Helper\ModelHelper;
use RK\ReferenceModule\Helper\SelectionHelper;
...
abstract class AbstractEditHandler
{
     ...
     * @param ReferenceFactory $entityFactory ReferenceFactory service instance
    ...
    public function __construct(
        ...
        ReferenceFactory $entityFactory,
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if (null === $this->returnTo) {
            ...
            if ($this->request->getSession()->has('rkreferencemoduleReferer')) {
                $this->returnTo = $this->request->getSession()->get('rkreferencemoduleReferer');
            } elseif ($this->request->headers->has('rkreferencemoduleReferer')) {
                $this->returnTo = $this->request->headers->get('rkreferencemoduleReferer');
                $this->request->getSession()->set('rkreferencemoduleReferer', $this->returnTo);
        ...
        $this->permissionComponent = 'RKReferenceModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'RKReferenceModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('rkreferencemodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logArgs = ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData($args)
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('RKReferenceModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
