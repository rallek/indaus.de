...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in RKReferenceModule.
     *
    ...
    public function getObjectTypes($context = '', $args = [])
    {
        ...
        $allowedObjectTypes = [];
        $allowedObjectTypes[] = 'attachedImage';
        $allowedObjectTypes[] = 'activity';
    
        return $allowedObjectTypes;
    }

    /**
     * Returns the default object type in RKReferenceModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $supportsHooks = false)
    {
        ...
        $showOwnEntries = $request->query->getInt('own', $this->variableApi->get('RKReferenceModule', 'showOnlyOwnEntries', 0));
        ...
        if ($showAllEntries != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('RKReferenceModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('RK\ReferenceModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $supportsHooks) {
            ...
            $currentUrlObject = new RouteUrl('rkreferencemodule_' . $objectType . '_' . /*$templateParameters['routeArea'] . */'view', $currentUrlArgs);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $supportsHooks = false)
    {
        ...
        if (true === $supportsHooks) {
            ...
            $currentUrlObject = new RouteUrl('rkreferencemodule_' . $objectType . '_' . /*$templateParameters['routeArea'] . */'display', $currentUrlArgs);
            $templateParameters['currentUrlObject'] = $currentUrlObject;
        }
