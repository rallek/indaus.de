...
namespace RK\ReferenceModule\Helper\Base;
...
abstract class AbstractUploadHelper
{
        ...
        $dataDirectory)
    {
        ...
        $this->allowedObjectTypes = ['attachedImage', 'activity'];
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        try {
            ...
            $this->logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}.', ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $this->logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if ($isImage) {
            ...
            if (true === $this->variableApi->get('RKReferenceModule', 'enableShrinkingFor' . $fieldSuffix, false)) {
                // check for maximum size
                $maxWidth = $this->variableApi->get('RKReferenceModule', 'shrinkWidth' . $fieldSuffix, 800);
                $maxHeight = $this->variableApi->get('RKReferenceModule', 'shrinkHeight' . $fieldSuffix, 600);
                ...
                if ($imgInfo[0] > $maxWidth || $imgInfo[1] > $maxHeight) {
                    ...
                    $preset = new \SystemPlugin_Imagine_Preset('RKReferenceModule_Shrinker', [
                        'width' => $maxWidth,
                        'height' => $maxHeight,
                        'mode' => 'inset'
                    ]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'RKReferenceModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
    ...
    {
        $basePath = $this->dataDirectory . '/RKReferenceModule/';
    
        switch ($objectType) {
            case 'attachedImage':
                $basePath .= 'attachedimages/addedimage/';
                break;
            case 'activity':
                $basePath .= 'activities/';
                switch ($fieldName) {
    ...
    public function checkAndCreateAllUploadFolders()
    {
        ...
        $result &= $this->checkAndCreateUploadFolder('attachedImage', 'addedImage', 'gif, jpeg, jpg, png');
    
        $result &= $this->checkAndCreateUploadFolder('activity', 'titleImage', 'gif, jpeg, jpg, png');
        $result &= $this->checkAndCreateUploadFolder('activity', 'referenceImage', 'gif, jpeg, jpg, png');
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$fs->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'RKReferenceModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'RKReferenceModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/RK/ReferenceModule/Resources/docs/htaccessTemplate';
        if (!$fs->exists($htaccessFilePath) && $fs->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'RKReferenceModule', 'directory' => $uploadPath]);
            }
        }
