 ...
 */
function rKInfoInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('rkinfomodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
...
function rKInfoTriggerFormValidation()
{
    rKInfoExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function rKInfoHandleFormSubmit (event) {
    if (triggerValidation) {
        rKInfoTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function rKInfoInitEditForm(mode, entityId)
{
    if (jQuery('.rkinfo-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.rkinfo-edit-form').first();
    ...
    editForm.submit(rKInfoHandleFormSubmit);
