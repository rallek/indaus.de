...
namespace RK\InfoModule\Form\Plugin\Base;
...
class AbstractItemSelector extends Zikula_Form_Plugin_TextInput implements ContainerAwareInterface
{
    ...
    public function render(Zikula_Form_View $view)
    {
        ...
        if ($firstTime) {
            ...
            $jsAssetBag->add($assetHelper->resolve('@RKInfoModule:js/RKInfoModule.Finder.js'));
        ...
        if (!$permissionApi->hasPermission('RKInfoModule:' . ucfirst($this->objectType) . ':', '::', ACCESS_COMMENT)) {
            return false;
        }

        $this->selectedItemId = $this->text;

        $repository = $this->container->get('rk_info_module.entity_factory')->getRepository($this->objectType);
    ...
    {
        parent::decode($view);
        $this->objectType = $this->container->get('request_stack')->getCurrentRequest()->request->get('RKInfoModule_objecttype', 'information');
        $this->selectedItemId = $this->text;
    }
}
