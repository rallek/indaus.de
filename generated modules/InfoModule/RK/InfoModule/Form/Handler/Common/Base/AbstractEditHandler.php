...
namespace RK\InfoModule\Form\Handler\Common\Base;
...
use RK\InfoModule\Helper\HookHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if (null === $this->returnTo) {
            ...
            if ($this->request->getSession()->has('rkinfomoduleReferer')) {
                $this->returnTo = $this->request->getSession()->get('rkinfomoduleReferer');
            } elseif ($this->request->headers->has('rkinfomoduleReferer')) {
                $this->returnTo = $this->request->headers->get('rkinfomoduleReferer');
                $this->request->getSession()->set('rkinfomoduleReferer', $this->returnTo);
        ...
        $this->permissionComponent = 'RKInfoModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'RKInfoModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'RKInfoModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('rkinfomodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logArgs = ['app' => 'RKInfoModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData($args)
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('RKInfoModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
